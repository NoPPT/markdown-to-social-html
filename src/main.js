...
async function getContentForGzh(html, customCss, highlightCss, macStyleCss) {
  ...
  const dom = new JSDOM(`<body><section id="hhmd">${html}</section></body>`);
  const document = dom.window.document;
  const hhmd = document.getElementById("hhmd");
        ...
        selectorList.forEach((selectorNode) => {
          ...
          if (selector === "#hhmd") {
            declarations.forEach((decl) => {
              ...
              hhmd.style[decl.property] = value;
            });
          } else {
            const elements = hhmd.querySelectorAll(selector);
  ...
  // 处理公式
  let elements = hhmd.querySelectorAll("mjx-container");
  elements.forEach((element) => {
  ...
  // 处理代码块
  elements = hhmd.querySelectorAll("pre code");
  elements.forEach((element) => {
  ...
  elements = hhmd.querySelectorAll("h1, h2, h3, h4, h5, h6, blockquote, pre");
  elements.forEach((element) => {
  ...
  hhmd.setAttribute("data-provider", "hhmd");
  return `${hhmd.outerHTML.replace(
    /class="mjx-solid"/g,
